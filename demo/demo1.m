...
data = importdata('Pendigits\data.txt');
...
try
    if alpha
        ...
        resultsDir = sprintf('experiment outcomes/%s/Seed_%d s_%d a_%.4f M_%d', datasetName, seedData.Seed, numSampleSets, alpha, M);
    else
        % Default setting: square root sampling
        alpha = 1 / sqrt(numPoints);
        sampleSize = round(sqrt(numPoints)); % Sample size of a sample set
        % Generate dynamic folder name
        resultsDir = sprintf('experiment outcomes/%s/Seed_%d s_%d a_sqrt M_%d', datasetName, seedData.Seed, numSampleSets, M);
    end
